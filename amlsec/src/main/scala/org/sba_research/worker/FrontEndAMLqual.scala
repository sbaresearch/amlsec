  ...
  def apply(workManager: ActorRef[SubmitWork], resultTopicActor: ActorRef[Topic.Command[WorkFinished]]): Behavior[Command] = Behaviors.setup { ctx =>
    Behaviors.setup { ctx =>
      Behaviors.withTimers { timers =>
        new FrontEndAMLqual(workManager, ctx, timers, resultTopicActor).start()
      }
    }
  }
...
class FrontEndAMLqual private(workManager: ActorRef[SubmitWork],
                              ctx: ActorContext[Command],
                              timers: TimerScheduler[Command],
                              resultTopicActor: ActorRef[Topic.Command[WorkFinished]]) {

  import FrontEndAMLqual._

  val config = Config()

  def start(): Behavior[Command] = {
    ctx.log.info("Selected front-end: AMLqual.")
    ctx.log.info("Subscribing to result topic.")
    resultTopicActor ! Topic.Subscribe(ctx.self)
    ctx.self ! StartRiskIdentification

    //timers.startSingleTimer("start", StartRiskIdentification, 70.seconds)
      ...
      case WorkFinished(work, result) =>
        ...
        result match {
          case FusekiDatasetCreationSuccessful =>
            ...
            busy(Map(WorkPushModelToRemoteDataset(work.jobId, nextWorkId(), config.amlConfig.filePath, Some(config.sfcConfig.sfcFilePath)) -> ToSubmit))
          case FusekiPushModelToRemoteDatasetSuccessful =>
            ...
            busy(Map(WorkAugmentModel(work.jobId, nextWorkId(), doAMLqual = true) -> ToSubmit))
          ...
          case InstantiationOfVulnerabilitiesSuccessful =>
            ...
            busy(Map(WorkGenerateQOPN(work.jobId, nextWorkId()) -> ToSubmit))
          case GenerationOfQOPNSuccessful =>
            PerformanceMeasurement.writeElapsedTimeToFile(Some("[Main] QOPN generation and case study END"), System.nanoTime(), config.debugConfig.outputPathPerformanceReport)
            ctx.log.info("Received work result (generation of QOPN and quality case study execution was successful).")
            idle()
          case _ => Behaviors.unhandled
        }
