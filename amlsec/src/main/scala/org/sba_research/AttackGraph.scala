...
object AttackGraph {
  ...
  def generate(config: Config, ontModel: OntModel): Option[Model] = {
  ...
  def plotPrunedAg(config: Config, model: OntModel): Unit = {

    val infModel = OntModelUtils.getInfModel(model)
    ...
    val queryResult = QueryExecutor.query(
      s =
        """
          ...
          |          ?assetType rdfs:subClassOf* secOnt:Asset.
          |          ?vulnerability a secOnt:Vulnerability.
          |          ?vulnerability secOnt:vulnerability_has_SeverityValue ?vulnScore.
  ...
  def getZoneFromAsset(config: Config, ontModel: OntModel, infModel: InfModel, asset: Individual): OntClass = {
    ...
    def get(res: Resource): OntClass = {
      ...
      /* Find a zone class from the individual's classes. */
      val cls = ieInvIndvClasses.find(zoneClasses.contains)
      cls match {
