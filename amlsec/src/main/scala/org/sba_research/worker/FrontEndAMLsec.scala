  ...
  def apply(workManager: ActorRef[SubmitWork], resultTopicActor: ActorRef[Topic.Command[WorkFinished]]): Behavior[Command] = Behaviors.setup { ctx =>
    Behaviors.setup { ctx =>
      Behaviors.withTimers { timers =>
        new FrontEndAMLsec(workManager, ctx, timers, resultTopicActor).start()
      }
    }
  }
...
class FrontEndAMLsec private(workManager: ActorRef[SubmitWork],
                             ctx: ActorContext[Command],
                             timers: TimerScheduler[Command],
                             resultTopicActor: ActorRef[Topic.Command[WorkFinished]]) {

  import FrontEndAMLsec._

  val config = Config()

  def start(): Behavior[Command] = {
    ctx.log.info("Selected front-end: AMLsec.")
